...
def inference(x, n_in, n_hiddens, n_out, is_training):
    ...
    def batch_normalization(shape, x, is_training, decay=0.999):
        """
        バッチ正規化を行う関数
        """
        ...
        train_mean = tf.assign(pop_mean,
                               pop_mean * decay + mean * (1 - decay))
        train_var = tf.assign(pop_var,
                              pop_var * decay + var * (1 - decay))
        with tf.control_dependencies([train_mean, train_var]):
            hoge = tf.nn.batch_normalization(x, mean, var,
                                             beta, gamma, eps)
        fuga = tf.nn.batch_normalization(x, pop_mean, pop_var,
                                         beta, gamma, eps)
        return tf.cond(is_training, lambda: hoge, lambda: fuga)
    ...
    for i, n_hidden in enumerate(n_hiddens):
        ...
        u = tf.matmul(input, W)
        h = batch_normalization([n_hidden], u, is_training=is_training)
        output = tf.nn.relu(h)
...
if __name__ == '__main__':
    ...
    y = inference(x, n_in=n_in, n_hiddens=n_hiddens,
                  n_out=n_out, is_training=is_training)
    loss = loss(y, t)
