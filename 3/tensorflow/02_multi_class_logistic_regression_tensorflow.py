...
# 識別率を出力するノード
identification_rate = tf.reduce_mean(tf.to_float(correct_prediction))
"""
モデル学習
"""
# 初期化
...
with tf.Session() as sess:
    ...
    for epoch in range(20):
        ...
        # エポックごとに交差エントロピーを出す．
        print('{:2d}'.format(epoch) + '回目   cross_entropy', end=':')
        print(cross_entropy.eval(session=sess, feed_dict={
            x: X,
            t: Y
        }), end='  ')
        print('識別率: {:.2f}'\
              .format(identification_rate.eval(session=sess,
                                               feed_dict={x: X,
                                                          t: Y})))
